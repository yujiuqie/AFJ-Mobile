...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if ([point isKindOfClass:[NSString class]]) {
        ...
        if ([point compare:CSToastPositionTop] == NSOrderedSame) {
            CGPoint point = CGPointMake(self.bounds.size.width / 2, (toast.frame.size.height / 2) + CSToastVerticalPadding);
            return point;
        } else if ([point compare:CSToastPositionBottom] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
        } else if ([point compare:CSToastPositionCenter] == NSOrderedSame) {
    ...
    return [self centerPointForPosition:CSToastPositionCenter withToast:toast];
}
