...
@interface AFJOSLogController ()

+ (AFJOSLogController *)sharedLogController;

@property(nonatomic) void (^updateHandler)(NSArray<AFJSystemLogMessage *> *);
...
@implementation AFJOSLogController

+ (void)load {
    // Persist logs when the app launches on iOS 10 if we have persistent logs turned on
    if (AFJOSLogAvailable()) {
//        if (NSUserDefaults.standardUserDefaults.AFJ_cacheOSLogMessages) {
...
+ (instancetype)sharedLogController {
    static AFJOSLogController *shared = nil;
...
+ (instancetype)withUpdateHandler:(void (^)(NSArray<AFJSystemLogMessage *> *newMessages))newMessagesHandler {
    AFJOSLogController *shared = [self sharedLogController];
    shared.updateHandler = newMessagesHandler;
    return shared;
}

- (id)init {
    NSAssert(AFJOSLogAvailable(), @"os_log is only available on iOS 10 and up");
...
- (BOOL)handleStreamEntry:(os_activity_stream_entry_t)entry error:(int)error {
    ...
    if (!error && entry) {
        if (entry->type == OS_ACTIVITY_STREAM_TYPE_LOG_MESSAGE ||
            ...
            // https://github.com/AFJTool/AFJ/issues/564
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                AFJSystemLogMessage *message = [AFJSystemLogMessage logMessageFromDate:date text:msg];
                if (self.persistent) {
