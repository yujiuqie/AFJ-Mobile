...
@property(nonatomic, strong) NSMutableArray<BMKClusterQuadtree *> *childrens;
...
- (void)addItem:(BMKQuadItem *)quadItem {
    ...
    if (!CGRectContainsPoint(_rect, quadItem.pt)) {
        return;
    }
...
- (NSArray<BMKQuadItem *> *)searchInRect:(CGRect)searchRect {
    ...
    if (!CGRectIntersectsRect(searchRect, _rect)) {
        return [NSArray array];
    }
    NSMutableArray<BMKQuadItem *> *array = [NSMutableArray array];
    // searchrect包含四叉树区域
