...
#import "BMKWalkRoutePlanViewController.h"
#import "BMKWalkRouteDetailViewController.h"
...
@interface BMKWalkRoutePlanViewController () <BMKMapViewDelegate, BMKRouteSearchDelegate, BMKFootViewDelegate, BMKRoutePlanSearchViewDelegate>
/// 当前界面的mapView
...
@implementation BMKWalkRoutePlanViewController
...
- (void)configUI {
    ...
    _footView = [[BMKFootView alloc] initWithFrame:CGRectMake(19 * widthScale, SCREENH - 19 * widthScale - 143.5 * widthScale - KiPhoneXSafeAreaDValue - kViewTopHeight, SCREENW - 19 * 2 * widthScale, 143.5 * widthScale)];
...
- (void)didClickDetailButton {
    BMKWalkRouteDetailViewController *controller = [[BMKWalkRouteDetailViewController alloc] init];
...
- (void)onGetWalkingRouteResult:(BMKRouteSearch *)searcher result:(BMKWalkingRouteResult *)result errorCode:(BMKSearchErrorCode)error {
    ...
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        //获取所有步行路线中第一条路线
        _routeLine = (BMKWalkingRouteLine *) result.routes.firstObject;
        //遍历步行路线中的所有路段
        [_routeLine.steps enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            //获取步行路线中的每条路段
            BMKWalkingStep *step = _routeLine.steps[idx];
            BMKWalkRouteDetailModel *model = [[BMKWalkRouteDetailModel alloc] init];
