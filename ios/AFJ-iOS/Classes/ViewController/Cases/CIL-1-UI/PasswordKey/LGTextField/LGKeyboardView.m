...
- (void)addNumberBoard {
    ...
    float top = 3.0;
    float btnWidth = (SCREENW - 2 * leftX - (rowCount - 1) * distanse) / rowCount;
...
- (void)addEnglishKeyboard:(NSString *)type {
    ...
    //第二排
    float leftX2 = (SCREENW - 9 * btnWidth - 8 * distanse) / 2;
    for (int i = 10; i < 19; i++) {
    ...
    //第三排按钮第一个离左边距的距离
    float leftX3 = (SCREENW - 7 * btnWidth - 6 * distanse) / 2;
    for (int i = 19; i < 26; i++) {
    ...
    defaultBtn.frame = CGRectMake(leftX, top + (distanseTop + btnHeight) * 3, SCREENW - leftX * 2 - distanse - 60, btnHeight);
    ...
    finishedButton.frame = CGRectMake(SCREENW - leftX - 60, top + (distanseTop + btnHeight) * 3, 60, btnHeight);
...
- (void)addOtherKeyboardView {
    ...
    //删除按钮
    UIButton *deleteButton = [UIButton buttonWithType:UIButtonTypeCustom];
    deleteButton.frame = CGRectMake(SCREENW - leftX - distanse - 2 * btnWidth, top + 2 * (btnHeight + distanseTop), 2 * btnWidth + distanse, btnHeight);
    [deleteButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [deleteButton setTitle:@"删除" forState:UIControlStateNormal];
    deleteButton.backgroundColor = DeleteBtnGrayColor;
    deleteButton.layer.masksToBounds = YES;
    ...
    //完成的按钮
    UIButton *finishedButton = [UIButton buttonWithType:UIButtonTypeCustom];
    finishedButton.frame = CGRectMake(SCREENW - leftX - 3 * btnWidth - 2 * distanse, top + (distanseTop + btnHeight) * 3, 3 * btnWidth + 2 * distanse, btnHeight);
    [finishedButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [finishedButton setTitle:@"完成" forState:UIControlStateNormal];
    finishedButton.backgroundColor = FinishedBtnBlueColor;
    finishedButton.layer.masksToBounds = YES;
