...
- (void)onLoginResponse:(int)errorCode {
    if (errorCode == 0) {
        ...
        // UI显示
        [[AFJIMViewController sharedIMViewController] refreshConnecteStatus];
        [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_green:[NSString stringWithFormat:@"登录成功,errorCode=%d", errorCode]];
    } else {
        NSLog(@"【DEBUG_UI】IM服务器登录/连接失败，错误代码：%d", errorCode);

        // UI显示
        [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_red:[NSString stringWithFormat:@"IM服务器登录/连接失败,code=%d", errorCode]];
    }
...
- (void)onLinkClose:(int)errorCode {
    ...
    // UI显示
    [[AFJIMViewController sharedIMViewController] refreshConnecteStatus];
    [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_red:[NSString stringWithFormat:@"与IM服务器的连接已断开! (%d)", errorCode]];
}
...
- (void)onKickout:(PKickoutInfo *)kickoutInfo {
    ...
    // 在信息列表中显示提示
    [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_red:alertContent];
    // 跳出一个Alert提示
    [Utils showAlert:@"你被踢了" content:alertContent btnTitle:@"知道了！" parent:[[AFJIMViewController sharedIMViewController] getMainViewController] handler:^(UIAlertAction *action) {
        // 退出应用
        [[[AFJIMViewController sharedIMViewController] getMainViewController] signOut:nil];
    }];
}
