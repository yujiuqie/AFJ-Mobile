...
- (void)messagesLost:(NSMutableArray *)lostMessages {
    ...
    // UI显示
    [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_brightred:[NSString stringWithFormat:@"[消息未成功送达]共%li条!(网络状况不佳或对方id不存在)", [lostMessages count]]];
}
...
- (void)messagesBeReceived:(NSString *)theFingerPrint {
    if (theFingerPrint != nil) {
        ...
        // UI显示
        [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_blue:[NSString stringWithFormat:@"[收到应答]%@", theFingerPrint]];
    }
}
