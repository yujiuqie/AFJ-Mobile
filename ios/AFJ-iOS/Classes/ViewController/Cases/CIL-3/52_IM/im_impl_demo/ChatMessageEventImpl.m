...
- (void)onRecieveMessage:(NSString *)fingerPrintOfProtocal withUserId:(NSString *)dwUserid andContent:(NSString *)dataContent andTypeu:(int)typeu {
    ...
    [[[AFJIMViewController sharedIMViewController] getMainView] makeToast:dataContent
                                                                 duration:3.0
                                                                 position:@"center"
                                                                    title:[NSString stringWithFormat:@"%@说：", dwUserid]
                                                                    image:[UIImage imageNamed:@"qzone_mark_img_myvoice.png"]];
    [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_black:[NSString stringWithFormat:@"%@说：%@", dwUserid, dataContent]];
}
...
- (void)onErrorResponse:(int)errorCode withErrorMsg:(NSString *)errorMsg {
    ...
    if (errorCode == ForS_RESPONSE_FOR_UNLOGIN) {
        ...
        [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_brightred:content];
    } else {
        NSString *content = [NSString stringWithFormat:@"Server反馈错误码：%d,errorMsg=%@", errorCode, errorMsg];
        [[[AFJIMViewController sharedIMViewController] getMainViewController] showIMInfo_red:content];
    }
}
